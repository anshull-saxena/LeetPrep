...
export const Component = () => {
  ...
  return (
    <div ref={containerRef} className="relative w-full overflow-hidden bg-black min-h-[300vh]">
      <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none z-0" />
      
      {/* Side menu */}
      <div ref={menuRef} className="fixed left-8 top-1/2 -translate-y-1/2 z-50 flex flex-col items-center gap-8" style={{ visibility: 'hidden' }}>
        <div className="flex flex-col gap-1.5 cursor-pointer group">
          <span className="w-6 h-0.5 bg-white/50 group-hover:bg-white transition-colors"></span>
          <span className="w-6 h-0.5 bg-white/50 group-hover:bg-white transition-colors"></span>
          <span className="w-6 h-0.5 bg-white/50 group-hover:bg-white transition-colors"></span>
        </div>
        <div className="[writing-mode:vertical-lr] rotate-180 text-[10px] font-black tracking-[0.5em] text-white/30 uppercase">SPACE</div>
      </div>

      {/* Main content */}
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen pointer-events-none px-6">
        <h1 ref={titleRef} className="text-[12vw] font-black tracking-tighter text-white leading-none text-center select-none">
          {splitTitle(titles[0])}
        </h1>
        
        <div ref={subtitleRef} className="mt-8 text-center text-white/70 space-y-2">
          <p className="subtitle-line text-lg md:text-2xl font-medium tracking-tight">
            {subtitles[0].line1}
          </p>
          <p className="subtitle-line text-lg md:text-2xl font-medium tracking-tight">
            {subtitles[0].line2}
      ...
      <div ref={scrollProgressRef} className="fixed bottom-12 left-12 right-12 z-50 flex items-center gap-8" style={{ visibility: 'hidden' }}>
        <div className="text-[10px] font-black tracking-[0.3em] text-white/50 uppercase">SCROLL</div>
        ...
        <div className="text-[10px] font-bold text-white/50 tabular-nums">
      ...
      <div className="relative z-0">
       {[...Array(totalSections)].map((_, i) => {
          const index = i + 1;
          return (
            <section key={i} className="h-screen flex flex-col items-center justify-center px-6">
              <h1 className="text-[12vw] font-black tracking-tighter text-white leading-none text-center select-none opacity-0">
                {titles[index]}
              </h1>
          
              <div className="mt-8 text-center text-white/70 space-y-2 opacity-0">
                <p className="text-lg md:text-2xl font-medium tracking-tight">
                  {subtitles[index].line1}
                </p>
                <p className="text-lg md:text-2xl font-medium tracking-tight">
                  {subtitles[index].line2}
                </p>
              </div>
            </section>
          );
